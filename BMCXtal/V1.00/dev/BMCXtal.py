...
class calibUI(QtWidgets.QDialog, calibui.Ui_Dialog):
    def __init__(self, parent=None):
        ...
        Dist0 = SpecVariable.SpecVariable('BMCPilatusDist', 'corvette.cars.aps.anl.gov:6789').getValue()
        self.T1.setText(str(Dist0))
        beamX0 = SpecVariable.SpecVariable('BMCPilatusBeamX', 'corvette.cars.aps.anl.gov:6789').getValue()
        self.T2.setText(str(beamX0))
        beamY0 = SpecVariable.SpecVariable('BMCPilatusBeamY', 'corvette.cars.aps.anl.gov:6789').getValue()
    ...
    def update_clicked(self):
        xtalcmd = SpecCommand.SpecCommandA('','corvette.cars.aps.anl.gov:6789')
        Dist = self.T1.text()
...
class mainUI(QtWidgets.QMainWindow, ui.Ui_MainWindow):
    ...
    def DataColl(self, Delt, Nu, KphiS, KphiE, StpN, TimNE):
        ...
        if flgon:
            ...
            if (int(StpN.text())>1) and (self.SUMCK.isChecked() == True):
                time.sleep(2)
                DummyPath = SpecVariable.SpecVariable('SCANLOG_PATH', 'corvette.cars.aps.anl.gov:6789').getValue()
                DummyN = SpecVariable.SpecVariable('SCAN_N', 'corvette.cars.aps.anl.gov:6789').getValue()
    ...
    def findfolder(self):
        ProjPath = SpecVariable.SpecVariable('STARTUP_PROJECT_PATH', 'corvette.cars.aps.anl.gov:6789').getValue()
        temp = ProjPath[25::]
        temp1 = 'T:'+temp+'images/'
    ...
    def lastscan(self, widget1, tempfile):
        ...
        LogPath = SpecVariable.SpecVariable('SCANLOG_PATH', 'corvette.cars.aps.anl.gov:6789').getValue()
        FilePath = SpecVariable.SpecVariable('SCANLOG_FILE', 'corvette.cars.aps.anl.gov:6789').getValue()
        len1 = len(LogPath)
    ...
    def nextscan(self, widget1, tempfile):
        ...
        LogPath = SpecVariable.SpecVariable('SCANLOG_PATH', 'corvette.cars.aps.anl.gov:6789').getValue()
        MajorPath = SpecVariable.SpecVariable('STARTUP_PROJECT_PATH', 'corvette.cars.aps.anl.gov:6789').getValue()
